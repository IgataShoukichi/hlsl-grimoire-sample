...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	light.ptColor.y = 10.0f;
	light.ptColor.z = 20.0f;
	...
	while (DispatchWindowMessage())
	{
		...
		if (light.ptRange >= 150)
		{
			pm = -1.0f;
		}
		else if (light.ptRange <= 10) {
			pm = 1.0f;
		}
		if (light.ptColor.x >= 20)
		{
			color_x = -1.0f;
		}
		else if (light.ptColor.x <= 0) {
			color_x = 1.0f;
		}
		if (light.ptColor.y >= 20)
		{
			color_y = -1.0f;
		}
		else if (light.ptColor.y <= 0) {
			color_y = 1.0f;
		}
		if (light.ptColor.z >= 20)
		{
			color_z = -1.0f;
		}
		else if (light.ptColor.z <= 0) {
			color_z = 1.0f;
		}

		light.ptRange += 0.5f * pm;
		light.ptColor.x += 1 * color_x;
		light.ptColor.y += 1 * color_y;
		light.ptColor.z += 1 * color_z;
